...
defineTypeNameAndDebug(Galbraith, 0);
addToRunTimeSelectionTable(precipitationModel, Galbraith, precipitationModel);
...
:
    ...
    precipitant_(this->speciesDict().lookup("precipitant")),
...
autoPtr<Galbraith> Galbraith::New
(
    volVectorField& U,
    volScalarField& rho,
    surfaceScalarField& phi
...
{
    return autoPtr<Galbraith>
    (
        new Galbraith(U, rho, phi)
    );
}
...
Foam::Galbraith::precipitationSource
(
    volScalarField& Y
) const
{
    ...
    Si = 3 * ka_ * Cg_ * m2 * rhod_ * (Wi/Ws_) * pow(this->speciesThermo().SI(), Ng_),
...
Foam::tmp<Foam::volScalarField>
Foam::Galbraith::alphaPrecipitationSource() const
{
    ...
    if(addAlphaSource_)
    {
        ...
        Alphai = 3 * ka_ * Cg_ * m2 * pow(this->speciesThermo().SI(), Ng_),
