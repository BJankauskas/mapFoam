...
defineTypeNameAndDebug(Oncul, 0);
addToRunTimeSelectionTable(precipitationModel, Oncul, precipitationModel);
...
Foam::Oncul::precipitationSource
(
    volScalarField& Y
) const
{
    ...
    Si = 3 * kv_ * Cg_ * rhod_ * (Wi/Ws_) * m2 * pow((this->speciesThermo().Sa() - 1), Ng_);
...
Foam::tmp<Foam::volScalarField>
Foam::Oncul::alphaPrecipitationSource() const
{
    ...
    if(addAlphaSource_)
    {
        ...
        Alphai == 3 * kv_ * Cg_ * m2 * pow((this->speciesThermo().Sa() - 1), Ng_);
